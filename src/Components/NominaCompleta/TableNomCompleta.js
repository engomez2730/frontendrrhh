...
import {GET_NOMINASCOMPLETAS_ACTION,cargarEmpleados,NOMINAS_COMPLETA_SELECIONADA_ACTION} from '../../actions/index'
...
const TableFinal = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text) => {
        ...
        return <>{newArreglo.map((newArreglo) => {
  ...
  const onClickModalCrear = (e,text) =>{
   ...
   return (
        <>
        <Button type='primary' key='crear' style={{marginLeft:'10px', marginTop:'90px'}} onClick={() => onClickModalCrear()}>Crear Nomina</Button>, 
          ...
          <Table 
           ...
           />
          <Modal title="Editar Nomina" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} width={1600}>
            <TablaLinea/>
          </Modal>
          <Modal title="Ver Nomina" open={isModalOpenVer} onOk={handleOkVER} onCancel={handleCancelVer} width={1000}>
            <VerNomina nomina={props?.nomina}/>
          </Modal>
          <Modal title="Crear Nomina" open={isModalOpenCrear} onOk={handleOkCrear} onCancel={handleCancelCrear} width={1000}>
            <CrearNomina/>
          </Modal>
        </>
   )
