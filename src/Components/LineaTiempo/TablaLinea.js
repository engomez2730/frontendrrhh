...
import {
  CAMBIAR_ESTADO,cargarEmpleados,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text,item) => {
        ...
        if(!nominaMes){
          return <Tag color='red' key={item._id}>Pendiente</Tag>
        }else if(nominaMes){
          return <Tag color='Green' key={item._id}>Completado</Tag>
        }
    ...
    },
    {
      ...
      width: 310,
      render: (text) => [
      <Button type='primary' key='ver' style={{marginLeft:'10px'}} onClick={e => onClickModal(e,text)}>Agregar Nomina</Button>, 
      <Button type='warning' key='manejar' style={{marginLeft:'10px'}} onClick={e => onClickModalVer(e,text)}>Ver Nomina</Button>, 
    ],
    },
  ];
  ...
  const empleados = props.empleados?.map((e,index) =>{
    ...
    return (
        <div>
           ...
           />
          <Modal title="Agregar Nomina" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} width={1000}>
            <CrearLinea/>
          </Modal>
          <Modal title="Editar Anuncio" open={isModalOpenVer} onOk={handleOkVER} onCancel={handleCancelVer} width={1000}>
             <VerNomina/>
          </Modal>
          <Modal title="Cambiar Estado" open={isModalOpenCrear} onOk={handleOkCrear} onCancel={handleCancelCrear} width={1000}>
            <CambiarEstado/>
          </Modal>

          <Button type='primary'onClick={(e) => onClickModalCrear(e)}>Actualizar Estado Nomina</Button>
        </div>
    );
}
